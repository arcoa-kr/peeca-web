    ...
    steps: [
      {
        ...
        title: '카드사 선택',
        desc: '사용 중인 카드사를 골라요.',
        src: '/assets/mockup/REG-company.png',
        alt: '카드사 선택 화면',
      },
      {
        number: '2',
        title: '카드이름 검색',
        desc: '인증 없이 카드이름만 검색하세요.',
        src: '/assets/mockup/REG-search.png',
        alt: '카드 검색 화면',
      },
      {
        number: '3',
        title: '등록 하기!',
        desc: '끝! 바로 사용할 수 있어요.',
        src: '/assets/mockup/REG-done.png',
        alt: '등록 완료 화면',
      },
    ],
  },
    ...
    steps: [
      {
        number: '1',
        title: '내 카드 목록',
        desc: '등록한 카드를 한눈에 볼 수 있어요.',
        src: '/assets/mockup/MGT-list.png',
        alt: '카드 목록 화면',
      },
      {
        number: '2',
        title: '카드 상세 보기',
        desc: '카드별 혜택을 확인할 수 있어요.',
        src: '/assets/mockup/MGT-detail.png',
        alt: '카드 상세 화면',
      },
      {
        number: '3',
        title: '카드 삭제',
        desc: '쉽게 삭제할 수 있어요.',
        src: '/assets/mockup/MGT-delete.png',
        alt: '카드 삭제 화면',
      },
    ],
  },
  ...
  return (
    <SectionWrapper id="how-it-works" bg="#ffffff" maxWidth="xl" center>
      ...
      >
        ...
        {/* 탭 */}
        <motion.div variants={item} className="flex gap-3 bg-(--color-bg) p-1.5 rounded-full">
          {TABS.map((tab, i) => (
            <button
              key={tab.id}
              onClick={() => setActive(i)}
            ...
            {current.steps.map((step) => (
              <div key={step.number} className="flex flex-col items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                  <span
                    className="w-7 h-7 rounded-full flex items-center justify-center text-sm font-semibold text-white bg-(--color-primary)/90"
                  >
                    {step.number}
                  </span>
                ...
                <img
                  src={step.src}
                  alt={step.alt}
                  className="w-full max-w-[230px] rounded-3xl shadow-md"
                />
              </div>
            ))}
          </motion.div>
